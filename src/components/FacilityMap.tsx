...
export const FacilityMap: React.FC<FacilityMapProps> = ({
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      const userMarker = L.marker([userLat, userLng], { icon: userIcon })
        .addTo(mapInstance.current)
        .bindPopup('<b class="text-xs">Your Current Location</b>');
  ...
  // Handle flyTo when selectedFacility changes
  useEffect(() => {
    if (mapInstance.current && selectedFacility) {
      mapInstance.current.flyTo([selectedFacility.lat, selectedFacility.lng], 15, {
        animate: true,
        duration: 1.5,
      });
